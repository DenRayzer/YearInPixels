...
class YearCollectionViewCell: UICollectionViewCell {
    ...
    func setSize() {
        ...
        let margin = 3
        let width = (Int(self.frame.width) - margin) / 12
        var count = 0
        ...
        scrollView.contentSize = CGSize(width: self.frame.width, height: CGFloat(width) * 31 + CGFloat(margin))
    }
...
extension YearCollectionViewCell: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.row != 0 {
            ...
            cell.day.status = color[indexPath.row - 1]
            presenter.setDay(day: cell.day)
        }
        let screenSize = UIScreen.main.bounds.size
